"""
Citation-Filtered Score Fusion v2 for FPPC opinions (experiment 009b).

Same as experiment 009 but with a lower circuit breaker threshold (1.2 vs 1.3).
Lower threshold makes the CB fire more aggressively — returning BM25-only for
more queries where BM25 has a clear winner — which should reduce dilution losses
while preserving citation pooling's rescue ability for ambiguous queries.
...
- Citation queries: build candidate pool from citation matches ∪ BM25 top-100,
  ...
  Circuit breaker fires when BM25 top1/top2 ratio >= 1.2, returning BM25 only.
"""
...
# Engine
# ---------------------------------------------------------------------------
class CitationScoreFusionV2(SearchEngine):
    """Citation-filtered score fusion v2: lower CB threshold (1.2)."""

    def __init__(self, cb_threshold: float = 1.2, w_bm25: float = 0.4,
                 w_sem: float = 0.6):
